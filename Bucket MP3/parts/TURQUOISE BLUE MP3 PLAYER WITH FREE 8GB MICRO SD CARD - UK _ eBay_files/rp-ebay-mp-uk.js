...
* Author:          Martin Hill
* Version:         1.2.1
* Description:     Custom tracking script for first party data & click tracking
**/
...
var opg = '';
try {
	opg = (typeof parent.document.referrer === 'string') ? parent.document.referrer : document.referrer;
} catch (e) {
...
var rpx_defaults = {
	...
	oif : location.href,
	opg : opg, //(typeof parent.document.referrer === 'string') ? parent.document.referrer : document.referrer,
	dck : '',
	dbg : false
}

// initialisation object allowed keys
var keys = ['clk','oif','opg','dck','dbg'];
...
{
	var kw = '', tr = '', el = '', dl = '';
	//kw = rpf(rqs(rpx_src,'kw'),'kw');
	tr = rqs(rpx_src,'tier');
	el = rqs(rpx_src,'eligible');
	dl = rqs(rpx_src,'deals');
...
rpx_params                         = {};
rpx_params.data                    = {};
rpx_params.data.u                  = (rqs(rpx_src,'rpg').indexOf('//www.ebay.co.uk') >= 0 ) ? rqs(rpx_src,'rpg') : rpx_url;
rpx_params.dfp                     = {};
rpx_params.dfp.u                   = rpx_src;
